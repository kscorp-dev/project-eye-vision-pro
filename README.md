...
2. 유저가 **시선으로 포인트를 바라보면** dwell 링이 채워짐
3. dwell 완료 후 **핀치(Pinch)로 확인** → 카메라가 전진하며 새로운 풍경이 펼쳐짐
4. **갈림길에서 시선+핀치로 방향을 선택**
5. 선택에 따라 **다른 루트/풍경 언락**
6. 눈 운동 세트 사이에 **목 운동 페이즈가 삽입**되어 복합 운동 (목 운동은 머리 움직임으로 자동 인식)
...
| 기능 | 제약 | 대응 방안 |
|------|------|-----------|
| 눈 좌표 직접 접근 | Apple 프라이버시 정책상 불가 | Device Anchor 방향 기반 + Look+Pinch 확인 방식 채택 |
...
| **MusicKit + MediaPlayer** | Apple Music / 팟캐스트 재생 |
| **StoreKit 2** | 인앱 결제 |
...
│   ├── SoundService            # 효과음
│   └── MusicService            # Apple Music / 팟캐스트 재생
...
### v0.5 - Look+Pinch 인터랙션 & 프라이버시 수정 (2026-04-15)
- **Look+Pinch 인터랙션** — 눈 운동 시 시선 dwell 후 핀치로 명시적 확인 (visionOS HIG 준수)
- **ExerciseView** — TapGesture + waitingForPinch 상태 + "핀치하여 확인" 프롬프트 UI
- **ExerciseWithMapView** — dwell→pinchConfirmed 2단계 확인 방식 적용
- **ExerciseImmersiveView** — SpatialTapGesture로 3D 엔티티 핀치 확인
- **목 운동 유지** — 머리 움직임 기반이므로 핀치 불필요, 기존 자동 인식 방식 유지
- **프라이버시 표현 정정** — "아이트래킹 데이터 수집" → "디바이스 방향(Device Anchor) 활용"으로 수정
- **개인정보 처리방침** — raw eye gaze 미사용 명시, 데이터 기기 내 처리 강조
- **기획서 동기화** — ExerciseType 8종, 프로그램 7종, Look+Pinch 플로우 반영

### v0.4 - 버그 수정 & Apple Music 연동 (2026-04-15)
- **Apple Music / 팟캐스트 재생** — MusicKit + MediaPlayer 기반 배경 음악, 운동 중 미니 플레이어 UI
- **MusicPickerSheet** — 이어서 재생, 운동용 플레이리스트, 팟캐스트 재개, 음악 검색 지원
- **운동 볼륨 자동 조절** — 운동 시작 시 볼륨 30%로 낮추고, 완료 시 70%로 복원
- **SoundService visionOS 호환** — AudioServicesPlaySystemSound → AVAudioPlayer + 번들 사운드 파일
- **ExerciseWithMapView 시간 추적 수정** — startTime 누락으로 totalExerciseTime이 갱신되지 않던 버그 수정
- **분기 선택/이벤트 보상 수정** — GameEngine.selectBranch()/completeEvent() 호출로 점수 보너스 정상 지급
- **업적 연동 수정** — AchievementsView가 stamp 대신 실제 업적 ID 기반으로 표시
- **ExerciseProgramTests 수정** — 프로그램 수 4→7 반영
- **SettingsView 버전 수정** — 1.0.0 → 0.4
- **DataStoreService 제거** — v0.3에서 .modelContainer()로 대체된 미사용 코드 삭제
...
- [x] Phase 9.5: 버그 수정 & Apple Music 연동 (v0.4)
- [x] Phase 10: 인터랙션 모델 수정 (Look+Pinch) (v0.5)
